...
| `impl Doctrine for Alias` rejected as alias nominality violation | ⚠️ | 📖 V4 | Promoted V4 Semantic Core contract: aliases remain compile-time substitutions, so doctrine impl targets must be nominal `shape` / `variant` types. The conformance guard expects `v4_ty_validate_alias_nominality`, `alias-nominality-diagnostics=1`, and `alias nominality` markers. |
