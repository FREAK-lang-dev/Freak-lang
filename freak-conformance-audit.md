...
**V4 expansion bootstrap checkpoint (2026-08-09):** Maverick now routes every bootstrap frontend query through `parse -> expand -> HIR`. The new `freak_expand` stage forwards syntax unchanged while recording stable ExpandedFile identity and deterministic provenance; its query, invalidation, component snapshot, 00-Unit v3 restore/manifest/diff/health, LSP wrappers, and architecture guards are executable. This is an architecture-only checkpoint with no language-semantic change: user-defined syntax rewriting, hygiene, gensyms, third-party execution, and public macro APIs remain absent, and the Alternative-4 annotation/macro boundary is unchanged.
