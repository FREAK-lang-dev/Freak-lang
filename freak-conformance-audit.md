...
| Borrowed returns `-> lend T` / `-> lend mut T` | ⚠️ | 📖 V4 — TY/MIR carry borrowed return shapes; Meiya validates direct and same-block local reloan provenance, rejects callee-owned escape and immutable-to-mutable upgrades, and conservatively rejects stored/forwarded call results until interprocedural loan propagation exists |
