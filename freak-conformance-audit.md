...
| Contract | Status | Verdict | Notes |
|---|---|---|---|
| `shape Foo { field: type }` declarations | ✅ | ✅ | V4 stores ordered field names, surface types, and exact name/type/segment spans in HIR; TY consumes stored facts without token reconstruction and retains global alias canonicalization. HIR snapshot v8 validates Shape-only owners (including empty Shapes), dense ordinals, counts, source ordering, and contained spans before atomic restore. Malformed-field recovery and unclosed-body behavior remain unchanged. Route fields and physical layout are separate follow-up slices. |
...
| `alias Matrix = [[num; 4]; 4]` type aliases | ⚠️ | 📖 V4 | V4 normalizes alias target type text and exact spans at the expanded-AST-to-HIR boundary, snapshots those HIR facts, and makes TY consume them without token reconstruction. Alias canonicalization through TY/MIR, cycle diagnostics, and editor/query facts remain covered; production V3 remains narrower. This is the first bounded No Syntax Past HIR family, not a claim that every type form or MIR body is syntax-free, and it does not make aliases nominal types. |
...
| `fixed pilot NAME: T = const_expr` root-level constants | ⚠️ | 📖 V4 | V4 stores declared surface type and exact type/name spans in HIR; TY reads these facts without token reconstruction, including inferred and recovered declarations. HIR v8 snapshots require a complete, validated Const record set before atomic restoration. Initializer inference/evaluation and cycle analysis remain syntax-facing and unchanged. Cycle detection, integer const chains/arithmetic, tuple/list/repeat-fill plus generic shape/route-constructor type inference, const task calls, constructor payload validation, and declared initializer mismatch diagnostics are in V4; full const-eval remains. |
...
| Lifetime parameters `'a` | ⚠️ | 📖 V4 — lexer/parser/TY preserve named lifetime binders and explicit ordinary-task `'long: 'short` relations on lend parameters/returns. Declared binders are reflexive, direct edges close transitively, cycles produce mutual reachability, and an iterative worklist handles converging/long graphs without recursive stack growth. Semantic/hover/definition facts cover declarations and outlives-bound references; repeated or forward bound references resolve to the declared binder, and distinct spans survive editor snapshots. Spanned diagnostics and full query invalidation/recomputation are covered. `'_` remains elided and cannot be declared or used as a bound; `'static` ordinary-task borrowed contracts remain reserved. Local fixed-layout storage and body-derived provenance/source discovery through reaching definitions are covered by the preceding rows; signature-derived lifetime solving beyond declared ordinary-task relations, general lexical region inference, and lifetime-bearing non-ordinary aggregate task boundaries remain open |
| Inferred lifetimes / elision | ⚠️ | 📖 V4 — direct/local reloan returns and statically resolved calls use deterministic signature-derived source sets. Elided shared returns collect all `lend`/`lend mut` parameters and elided mutable returns collect only `lend mut` parameters; named returns apply the same mode rule plus reflexive/direct/transitive outlives eligibility. Candidate selection deliberately ignores top-level pointee type. Body-derived provenance/source discovery traces eligible candidates through local aliases, aggregate builds and moves, stored and constructor-direct call results, projection rebinding, acyclic joins, and bounded loop fixed points. Stored elided multi-owner results keep every candidate owner live through final reachable holder use; and source edits that change eligible signature sources, aggregate children, or a loop backedge invalidate and recompute TY/MIR/borrowck/diagnostic/editor queries. Lifetime eligibility remains signature-derived from declared ordinary-task contracts; general lexical region inference, non-ordinary aggregate task boundaries, unsupported carriers, and non-ordinary forwarding remain open |
...
all 18 report fields, comprising 15 concrete query families plus
...
lifetime-bearing task, edits the returned field use, checks all 18 invalidation
...
Contract diagnostics retain normalized source paths and exact byte spans.
Body-derived provenance/source discovery through reaching definitions is
implemented. The checkpoint does not claim signature-derived lifetime solving
beyond declared ordinary-task relations, general lexical region inference,
`'static`, non-ordinary aggregate forwarding, dynamic/wrapper storage,
index-sensitive array contracts, a runtime aggregate-loan ABI, or production
backend lowering.
...
append-only arena reclamation remains open. Lifetime eligibility remains
signature-derived from declared ordinary-task contracts; body-derived
provenance/source discovery then resolves concrete returned-loan paths through
reaching definitions. Loop provenance uses iterative
