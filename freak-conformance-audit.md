...
| `fixed pilot NAME: T = const_expr` root-level constants | ⚠️ | 📖 V4 | cycle detection works; integer const chains/arithmetic, tuple/list/repeat-fill plus shape/route-constructor type inference, constructor payload validation, and declared initializer mismatch diagnostics are in V4, full const-eval remains |
