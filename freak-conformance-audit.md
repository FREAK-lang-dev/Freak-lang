...
| Raw pointer ops (`*ptr`, `.read()`, `.offset()`, `.cast<U>()`, `.is_null()`) | ⚠️ | 📖 V4 | V4 lowers `.is_null()` to LLVM `icmp eq ptr %p, null` outside `trust me` (bible §16.4 explicitly permits null-checks anywhere), `*ptr` reads to LLVM `load <pointee>, ptr %op` gated on a surrounding `.cadet+` `trust me` block (`raw-pointer deref needs trust me block` diagnostic for derefs outside one), and `*ptr = value` writes to LLVM `store <pointee> <value>, ptr <ptr>` with `.pilot+` honor gating plus a `raw-pointer write needs *mut T` diagnostic for writes through `*const T`/`*T`; the method forms `.read()`/`.write()`/`.offset()`/`.cast<U>()`, allocation, and freeing are still 🔜 V4 |
