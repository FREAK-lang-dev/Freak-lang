...
| Borrowed returns `-> lend T` / `-> lend mut T` | ⚠️ | 📖 V4 — TY/MIR carry borrowed return shapes; Meiya validates direct and same-block local reloan provenance, rejects callee-owned escape and immutable-to-mutable upgrades; interprocedural forwarded-loan proof awaits region solving |
