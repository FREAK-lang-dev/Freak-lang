...
| 6 | Full borrow checker: `lend`/`lend mut`, `'a` lifetimes, `Shared<T>`/`Weak<T>` | v0.13.x has Phase-1 mut/move checking; V4 now carries direct/transitive ordinary-task outlives bounds, deterministic named/elided returned-loan source sets, and the partial Meiya provenance/liveness work itemized in §4. Body-derived/general lexical inference, aggregate storage, non-ordinary forwarding, loop fixed points, `'static`, and runtime shared ownership remain | 🟡 | 📖 split into Phase-1 (current) + V4 sections |
...
| Borrowed returns `-> lend T` / `-> lend mut T` | ⚠️ | 📖 V4 — TY/MIR carry elided and explicit `-> lend 'a T` / `-> lend mut 'a T` shapes. Ordinary-task named returns deterministically select every mode-compatible parameter whose lifetime equals or outlives the return lifetime; elided returns select every mode-compatible borrowed parameter. Shared returns accept `lend` and `lend mut`, while mutable returns accept only `lend mut`. Meiya validates origins through projections, scalar holders, statically resolved ordinary calls, reordered args, and acyclic CFG joins. Caller `ReturnLoan` / `ReturnLoanMut` paths keep all candidate owners live through the final reachable holder use and survive snapshots/invalidation. Callee-owned escape, immutable-to-mutable upgrades, missing outlives relations, malformed targets, `'static`, and aggregate storage diagnose. Method/dynamic/callback/closure/extern/FFI forwarding and loop fixed points remain |
...
| Lifetime parameters `'a` | ⚠️ | 📖 V4 — lexer/parser/TY preserve named lifetime binders and direct/transitive ordinary-task `'long: 'short` relations on lend parameters/returns; semantic/hover/definition facts cover declarations and outlives-bound references, alongside spanned diagnostics, editor snapshots, and query invalidation. `'_` remains elided and cannot be declared or used as a bound; `'static` ordinary-task borrowed contracts remain reserved. Body-derived/general lexical solving and stored lifetime-bearing values remain |
| Inferred lifetimes / elision | ⚠️ | 📖 V4 — direct/local reloan returns and statically resolved ordinary calls use deterministic signature-derived source sets. Elided shared returns collect all `lend`/`lend mut` parameters and elided mutable returns collect only `lend mut` parameters; named returns apply the same mode rule plus equal/direct/transitive outlives eligibility. Candidate selection deliberately ignores top-level pointee type and keeps every candidate owner live through final reachable holder use. This is not body-derived/general lexical inference; aggregate storage, non-ordinary forwarding, and loop-carried fixed points remain |
...
- §4 borrow checker: V4 contract-region fixture sources now encode repeated-source named/elided sets, direct/transitive outlives relations, shared/mutable mode filtering, projection/scalar-holder/statically resolved ordinary-call/reordered-argument/acyclic-join provenance, all-candidate final-use liveness, and malformed/storage boundaries. Harness registration and dedicated semantic/hover/definition assertions on outlives-bound references remain test gaps at this prerequisite, alongside body-derived/general lexical inference, aggregate/shape/route/container storage, method/dynamic/callback/closure/extern/FFI forwarding, loop-carried fixed points, `'static`, and runtime ownership depth
