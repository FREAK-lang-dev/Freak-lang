...
def build_academy_reference(root: Path | None = None) -> dict[str, Any]:
    return _load_json(reference_path(root))


def export_academy_reference(destination: Path, root: Path | None = None) -> None:
    destination.parent.mkdir(parents=True, exist_ok=True)
    reference = build_academy_reference(root=root)
...
def format_course_listing(root: Path | None = None) -> str:
    ...
    lines.extend(
        [
            ...
            "  freak learn check-examples [--source all|lesson|reference|book]",
