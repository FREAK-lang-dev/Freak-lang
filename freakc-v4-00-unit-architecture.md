...
reinterpreted. Borrowck restore discards active provenance scratch and starts a
fresh generation. The fixed-aggregate query smoke checkpoints A, edits to B,
...
`T`. Body-derived source discovery, general lexical region inference, and
`'static` classification are still open. Dynamic/wrapper storage and any
backend or runtime aggregate-loan ABI remain beyond this checkpoint.
