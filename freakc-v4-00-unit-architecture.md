...
Meiya keys aggregate provenance by the rvalue and requested projection. Local aggregate
holder aliases preserve that projection path. A later `.0`, `.field`, or
...
indices conservatively union every possible child. `LoanMut` remains exclusive
against overlapping owner observations, writes, moves, and loans while that
projected holder is live. Repeat-filling multiple slots from one `lend mut` is
diagnosed because one exclusive loan cannot be cloned into a formation.

These facts use the existing tooling protocols. MIR and borrowck facts serialize
through their current component snapshots and the 14-section 00-Unit envelope;
restore, manifest, diff, and health do not gain aggregate-only formats. Source
changes still report all 17 invalidation fields and recompute the existing TY,
MIR, borrowck, diagnostics, editor, and query families. No new LSP method is
required.
...
aggregate parameters or returns do not carry fixed-layout provenance. Generic
task-call substitution cannot smuggle a lend through `T`. Body-derived source
discovery, general lexical region inference, and `'static` classification are
still open.
