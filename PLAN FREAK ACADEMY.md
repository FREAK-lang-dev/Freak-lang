...
- Goal E first WASM evaluator is staged: `freak learn wasm-evaluator` builds `academy-wasm-evaluator.wasm`, the browser worker can consume it through `wasmEvaluator`, and `hello-freak` plus `variables` can be evaluated through WASM while other lessons keep the JavaScript reference fallback.

The next active goal should continue Goal E by expanding the WASM evaluator beyond `variables`, then package the evaluator in the browser asset manifest once it covers enough of the basics course to be useful on `freaklang.dev`.
