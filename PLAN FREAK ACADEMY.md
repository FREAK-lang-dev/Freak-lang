...
- Goal E first WASM evaluator is staged: `freak learn wasm-evaluator` builds `academy-wasm-evaluator.wasm`, the browser worker can consume it through `wasmEvaluator`, and the first four basics lessons (`hello-freak`, `variables`, `primitive-types`, `arithmetic`) can be evaluated through WASM while later lessons keep the JavaScript reference fallback.

The next active goal should continue Goal E by expanding the WASM evaluator into conditions and loops, then package the evaluator in the browser asset manifest once it covers enough of the basics course to be useful on `freaklang.dev`.
