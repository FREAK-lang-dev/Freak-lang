...
| §4 Borrow Checker | ⚠️ Partial | Phase-1 BC ships behind `--strict-borrow`: mutability + single-owner moves + Copy/Move types. V4 now carries lifetime tokens/type contracts and initial `lend` / `lend mut` parameter contracts through TY/MIR/Meiya; full region solving, `Shared<T>`/`Weak<T>`, honor levels, and `direct_order` remain V4. |
...
> writes, blocks moves out of borrowed parameters, and keeps borrowed params
> out of callee drop tracking. Lifetime tokens and signature contracts exist,
> but full region inference, `Shared<T>` / `Weak<T>` / `.borrow()` /
> `.borrow_mut()` / `.get_mut()`, the full honor-level system inside
> `trust me ... on my honor as .level`, and `direct_order [arch] { asm }`
> inline assembly are V4 work. This is the first parameter-contract slice,
