...
RUNTIME_STATS_PREFIX = "FREAK_RUNTIME_STATS_V1 "
...
_PROFILE_SPECS = {
    "O0": {"id": "opt0", "display": "O0", "arguments": ["--opt=0"]},
    "O1": {"id": "opt1", "display": "O1", "arguments": ["--opt=1"]},
    "O2": {"id": "opt2", "display": "O2", "arguments": ["--opt=2"]},
    "O3": {"id": "opt3", "display": "O3", "arguments": ["--opt=3"]},
    "+03": {"id": "plus03", "display": "+03 — FINAL FORM", "arguments": ["+03"]},
}
...
def _strip_runtime_stats(stderr: str) -> tuple[str, dict[str, Any] | None, list[str]]:
    ...
    for line in stderr.splitlines(keepends=True):
        ...
        if not isinstance(record, dict):
            failures.append("runtime stats sentinel payload must be an object")
            continue
...
def _one_result(
    *,
    ...
    for _ in range(warmups):
        ...
        if warmed.returncode != mode["expected_exit_code"]:
            raise LabError(f"warmup failed for {case['id']}/{backend}/{profile}")
    ...
    for sample_index in range(samples):
        ...
        stdout = _canonical_output(completed.stdout, "benchmark stdout")
        raw_stderr = _canonical_output(completed.stderr, "benchmark stderr")
        stderr, counter, counter_failures = _strip_runtime_stats(raw_stderr)
        sample_failures = list(counter_failures)
        if completed.returncode != mode["expected_exit_code"]:
            sample_failures.append(
                f"exit code {completed.returncode}, expected {mode['expected_exit_code']}"
            )
        if stdout != mode["expected_stdout"]:
            sample_failures.append("stdout differs from manifest")
        if stderr != mode["expected_stderr"]:
            sample_failures.append("stderr differs from manifest after stats removal")
        if _sha256_text(stdout) != mode["expected_stdout_sha256"]:
            sample_failures.append("stdout checksum differs from manifest")
        if _sha256_text(stderr) != mode["expected_stderr_sha256"]:
            sample_failures.append("stderr checksum differs from manifest")
    ...
    if len(present_counters) == len(counters):
        ...
        runtime_counters_reason = (
            ...
            if present_counters
            else "runtime stats sentinel not emitted by this build"
        )
    ...
    return {
        ...
        "binary": {"bytes": binary.stat().st_size, "sha256": _sha256_file(binary)},
        "run": {"samples": run_samples, "median_ns": int(statistics.median(durations))},
        "peak_rss_bytes": None,
        "peak_rss_reason": "portable per-child peak RSS collection is unavailable in lab schema v1",
...
def run_lab(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="freak-v3-performance-lab-") as temporary:
        ...
        compiler, help_text = _compiler_identity(cli, environment, work_dir, args.timeout)
        toolchain = _tool_identity(clang, environment, work_dir, args.timeout)
...
def validate_output(path: Path, *, cli_override: str | None = None) -> dict[str, Any]:
    ...
    _exact_keys(
        compiler,
        {"path", "sha256", "bytes", "version", "version_stdout", "version_stderr", "version_output_sha256"},
        "result.compiler",
    )
    ...
    if any(profile not in _PROFILE_SPECS for profile in available_profiles) or not set(profiles) <= set(available_profiles):
    ...
    for index, raw_result in enumerate(results):
        ...
        _exact_keys(compile_info, {"duration_ns", "stdout_sha256", "stderr_sha256"}, f"{context}.compile")
        _require_int(compile_info.get("duration_ns"), f"{context}.compile.duration_ns", minimum=0)
        for channel in ("stdout", "stderr"):
            if not re.fullmatch(r"[0-9a-f]{64}", str(compile_info.get(f"{channel}_sha256", ""))):
                raise LabError(f"{context} compiler {channel} checksum is invalid")
        binary = _require_dict(result.get("binary"), f"{context}.binary")
        _exact_keys(binary, {"bytes", "sha256"}, f"{context}.binary")
        _require_int(binary.get("bytes"), f"{context}.binary.bytes", minimum=1)
        if not re.fullmatch(r"[0-9a-f]{64}", str(binary.get("sha256", ""))):
            raise LabError(f"{context} binary checksum is invalid")
        ...
        for sample_index, raw_sample in enumerate(raw_samples):
            ...
            for channel in ("stdout", "stderr"):
                ...
                raw_checksum = sample.get(f"{channel}_raw_sha256")
                if not isinstance(raw_checksum, str) or not re.fullmatch(r"[0-9a-f]{64}", raw_checksum):
                    raise LabError(f"{context} sample raw {channel} checksum is invalid")
        if run.get("median_ns") != int(statistics.median(durations)):
            raise LabError(f"{context} median does not match raw samples")
        if result.get("peak_rss_bytes") is None and not result.get("peak_rss_reason"):
            raise LabError(f"{context} lacks a peak RSS reason")
        if result.get("peak_rss_bytes") is not None:
            _require_int(result.get("peak_rss_bytes"), f"{context}.peak_rss_bytes", minimum=0)
        if result.get("runtime_counters") is None and not result.get("runtime_counters_reason"):
            raise LabError(f"{context} lacks a runtime counters reason")
        if result.get("runtime_counters") is not None:
            counters = _require_dict(result.get("runtime_counters"), f"{context}.runtime_counters")
            counter_samples = _require_list(counters.get("samples"), f"{context}.runtime_counters.samples")
            if len(counter_samples) != samples or not all(isinstance(sample, dict) for sample in counter_samples):
                raise LabError(f"{context} runtime counter samples are invalid")
